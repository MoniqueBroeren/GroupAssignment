...
def make_correlation_matrix(df_features, figurename):
    ...
    Parameters: df_features (DataFrame) - contains all calculated features
                figurename (str)        - name that is wanted for above correlation matrix heatmap
    Returns:    -
    """
    ...
    sns.heatmap(absolute_correlation_matrix, cmap='coolwarm', annot=False)
...
def cumulative_explained_variance_graph(cumulative_variance, label, cum_var_threshold=0.8):
    ...
    plt.title(f'Cumulative explained variance for each principal component for {label}')
    plt.savefig(f'results/cumlative_explained_variance_graph for {label}.svg')
...
def prediction_and_validation(input_file, use_descriptors, use_fingerprints, add_binarized_columns, cum_var_threshold, label, resample_method, classifier_type, iterations):
    """ Runs code for training the model and validates this model. Based on this code, the best model can be selected
    for predicting the untested molecules labels.

    Parameters: input_file (str)                - path where the file is located
                use_descriptors (Boolean)       - adds descriptors as feature of molecule if set True
                use_fingerprints (Boolean)      - adds fingerprints as feature of molecule if set True
                add_binarized_columns (Boolean) - add binarized columns as feature of molecule if set True
                cum_var_threshold (float)       - minimal cumulative explained variance of PCs together
                label (str)                     - what needs to be predicted ('PKM2_inhibition' or 'ERK2_inhibition')
                resample_method (str)           - resampling method ('random_undersample', 'random_oversample', 'SMOTE', 
                                                  'near_miss', 'smote_and_near_miss' or 'no_resampling')   
                ...
                iterations (int)                - number of iterations over which evaluation metrics need to be averaged
    Returns:    selected_features (list)        - contains all features that are used for machine learning
                pca (object)                    - pca object with wanted amount of cumulative explained variance
                num_components (int)            - number of PCs needed for wanted cumulative explained variance
                df_pc_scores_labels (DataFrame) - contains all PCs scores with information that contains
                                                  at least a threshold cumulative explained variance together and
                                                  labels
    """
    # read file
    ...
    make_correlation_matrix(df_features.iloc[:,1:], f'Correlation matrix of all features for {label}')

    # only keep important columns
    selected_features, df_features = prefilter_irrelevant_columns(df_all_info, df_features, label)
    make_correlation_matrix(df_features, f'Correlation matrix of filtered features for {label}')
    ...
    cumulative_explained_variance_graph(cumulative_variance, label, cum_var_threshold)
    ...
    return selected_features, pca, num_components, df_pc_scores_labels
...
if __name__ == '__main__':
    ...
    # VARIABLES FOR PKM2
    use_descriptors_PKM2 = True
    use_fingerprints_PKM2 = False
    add_binarized_columns_PKM2 = False
    cum_var_threshold_PKM2 = 0.8
    label_PKM2 = 'PKM2_inhibition' # options: 'PKM2_inhibition', 'ERK2_inhibition' 
    resample_method_PKM2 = 'smote_and_near_miss' # options: 'random_undersample', 'random_oversample', 'SMOTE', 'near_miss', 'smote_and_near_miss', 'no_resampling'
    classifier_type_PKM2 = 'SVM' # options: 'DT', 'RF', 'LR', 'SVM'
    iterations_PKM2 = 50

    selected_features_PKM2, pca_PKM2, num_components_PKM2, df_pc_scores_labels_PKM2 = prediction_and_validation(input_file, use_descriptors_PKM2, use_fingerprints_PKM2, add_binarized_columns_PKM2, 
                                                                                                                cum_var_threshold_PKM2, label_PKM2, resample_method_PKM2, classifier_type_PKM2, iterations_PKM2)
    
    # VARIABLES FOR ERK2
    use_descriptors_ERK2 = True
    use_fingerprints_ERK2 = False
    add_binarized_columns_ERK2 = False
    cum_var_threshold_ERK2 = 0.8
    label_ERK2 = 'ERK2_inhibition' # options: 'PKM2_inhibition', 'ERK2_inhibition' 
    resample_method_ERK2 = 'smote_and_near_miss' # options: 'random_undersample', 'random_oversample', 'SMOTE', 'near_miss', 'smote_and_near_miss', 'no_resampling'
    classifier_type_ERK2 = 'SVM' # options: 'DT', 'RF', 'LR', 'SVM'
    iterations_ERK2 = 50
    selected_features_ERK2, pca_ERK2, num_components_ERK2, df_pc_scores_labels_ERK2 = prediction_and_validation(input_file, use_descriptors_ERK2, use_fingerprints_ERK2, add_binarized_columns_ERK2, 
                                                                                                                cum_var_threshold_ERK2, label_ERK2, resample_method_ERK2, classifier_type_ERK2, iterations_ERK2)
    ...
    new_pca_scores = pca_PKM2.transform(df_features_scaled_new)
    df_new_pc_scores = pd.DataFrame(new_pca_scores, columns=[f'PC{i+1}' for i in range(num_components_PKM2)])

    results = {'SMILES': df_all_info_new.iloc[:,0]} # make resulting dictionary
    X_train, y_train = split_data(df_pc_scores_labels_PKM2, label_PKM2, resample_method_PKM2, molecules='untested')
    y_pred = classifier(X_train, np.array(df_new_pc_scores), y_train, classifier_type_PKM2)
    ...
    df_features_scaled_new = scale_data(df_features_new)
    new_pca_scores = pca_ERK2.transform(df_features_scaled_new)
    df_new_pc_scores = pd.DataFrame(new_pca_scores, columns=[f'PC{i+1}' for i in range(num_components_ERK2)])

    X_train, y_train = split_data(df_pc_scores_labels_ERK2, label_ERK2, resample_method_ERK2, molecules='untested')
    y_pred = classifier(X_train, np.array(df_new_pc_scores), y_train, classifier_type_ERK2)
    results[label_ERK2] = y_pred # add prediction ERK2 to resulting dictionary
